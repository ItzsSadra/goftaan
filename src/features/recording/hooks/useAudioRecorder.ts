// Audio recorder hook - manages recording lifecycle
...
const useNativeAudioRecorder = (meetingId: string): UseAudioRecorderResult => {
  const recorder = useExpoAudioRecorder(RecordingPresets.HIGH_QUALITY) as NativeAudioRecorder;
...
// Web implementation
const useWebAudioRecorderWrapper = (meetingId: string): UseAudioRecorderResult => {
  ...
  const durationMs = webRecorder.durationMs;

  // Map web recorder status to our status
  useEffect(() => {
    switch (webRecorder.status) {
      case 'idle':
        setStatus('idle');
        break;
      ...
      case 'stopping':
        ...
        break;
      case 'completed':
        // Result is handled in stop callback
        break;
      case 'error':
        setStatus('error');
        break;
  ...
  const stop = useCallback(async () => {
    try {
      ...
      const localUri = await audioRecordingService.persistWebRecordingBlob(blob, meetingId);

      setResult({
        localUri,
        durationMs: webRecorder.durationMs,
      });
      setStatus('completed');
    } catch (error) {
      setStatus('error');
  ...
  return {
    ...
    durationMs,
    result,
    errorMessage: errorMessage || webRecorder.errorMessage,
    interruptionMessage,
    start,
    pause,
    resume,
...
// Main hook that switches between native and web implementations
export const useAudioRecorder = (meetingId: string): UseAudioRecorderResult => {
  const isWeb = Platform.OS === 'web';
  
  const nativeResult = useNativeAudioRecorder(meetingId);
  const webResult = useWebAudioRecorderWrapper(meetingId);
